(function (global, factory) {
    ...
    FormatNumber.formatNumberWithFormat = function (rawvalue, format_string, currency_char) {
        ...
        currency_char = currency_char || _formulaConstants.DefaultCurrency;
        ...
        if (sectioninfo.hasdate) {
            ...
            startval = (rawvalue - Math.floor(rawvalue)) * _formulaConstants.SecondsInDay; // get date/time parts
            estartval = rawvalue * _formulaConstants.SecondsInDay; // do elapsed time version, too
            hrs = Math.floor(startval / _formulaConstants.SecondsInHour);
            ehrs = Math.floor(estartval / _formulaConstants.SecondsInHour);
            startval = startval - hrs * _formulaConstants.SecondsInHour;
            ...
            ymd = FormatNumber.convert_date_julian_to_gregorian(Math.floor(rawvalue + _formulaConstants.JulianOffset));
            ...
            for (;; mspos++) {
                ...
                if (op == scfn.commands.date) {
                    if ((operandstr.toLowerCase() == "am/pm" || operandstr.toLowerCase() == "a/p") && !ampmstr) {
                        if (hrs >= 12) {
                            hrs -= 12;
                            ampmstr = operandstr.toLowerCase() == "a/p" ? _formulaConstants.PM1 : _formulaConstants.PM; // "P" : "PM";
                        } else {
                            ampmstr = operandstr.toLowerCase() == "a/p" ? _formulaConstants.AM1 : _formulaConstants.AM; // "A" : "AM";
                        }
        ...
        textcolor = "";
        textstyle = "";
        separatorchar = _formulaConstants.SeparatorChar;
        if (separatorchar.indexOf(" ") >= 0) separatorchar = separatorchar.replace(/ /g, "&nbsp;");
        decimalchar = _formulaConstants.DecimalChar;
        ...
        while (op = format.operators[oppos]) {
            ...
            if (op == scfn.commands.copy) {
                ...
                if (operandstrlc == "y" || operandstrlc == "yy") {
                    ...
                    result += _formulaConstants.DayNames3[cval];
                } else if (operandstrlc == "dddd") {
                    cval = Math.floor(rawvalue + 6) % 7;
                    result += _formulaConstants.DayNames[cval];
                    ...
                    result += _formulaConstants.MonthNames3[ymd.month - 1];
                } else if (operandstrlc == "mmmm") {
                    result += _formulaConstants.MonthNames[ymd.month - 1];
                } else if (operandstrlc == "mmmmm") {
                    result += _formulaConstants.MonthNames[ymd.month - 1].charAt(0);
    ...
    FormatNumber.parse_format_bracket = function (bracketstr) {
        ...
        if (bracketstr.charAt(0) == "$") {
            ...
            if (parts) {
                bracketdata.operand = parts[1] || _formulaConstants.DefaultCurrency || "$";
            } else {
                bracketdata.operand = bracketstr.substring(1) || _formulaConstants.DefaultCurrency || "$";
            }
        } else if (bracketstr == "?$") {
            bracketdata.operator = scfn.commands.currency;
            bracketdata.operand = "[?$]";
        } else if (_formulaConstants.AllowedColors[bracketstr.toUpperCase()]) {
            bracketdata.operator = scfn.commands.color;
            bracketdata.operand = _formulaConstants.AllowedColors[bracketstr.toUpperCase()];
